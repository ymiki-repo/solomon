...
function plot_series(file_list, summary, GPU, lang, mode, max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med, fig_max, ax_max, fig_med, ax_med)
    ...
    for arg in [(opt="O1", point_id=1, line_id=1, color_id=2), (opt="O2", point_id=2, line_id=2, color_id=3), (opt="O3", point_id=3, line_id=3, color_id=0), (opt="Ofast", point_id=0, line_id=0, color_id=1)]
        ...
        plot_lp(fig_max, ax_max, num, perf_max, point_id=arg.point_id, line_id=arg.line_id, color_id=arg.color_id, label=arg.opt)
        plot_lp(fig_med, ax_med, num, perf_med, point_id=arg.point_id, line_id=arg.line_id, color_id=arg.color_id, label=arg.opt)
    end
...
function main()
    ...
    for gpu_id in 1:Ngpu
        ...
        summary, max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med = plot_series(glob(string(root, "/iterate/model0/diffusion_benchmark_*.csv")), summary, gpu[gpu_id].caption, "omp (loop)", "data", max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med, compare_max, compare_max.ax[1, Ngpu-gpu_id+1], compare_med, compare_med.ax[1, Ngpu-gpu_id+1])
        summary, max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med = plot_series(glob(string(root, "/iterate/model1/diffusion_benchmark_*.csv")), summary, gpu[gpu_id].caption, "omp (distribute)", "data", max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med, compare_max, compare_max.ax[2, Ngpu-gpu_id+1], compare_med, compare_med.ax[2, Ngpu-gpu_id+1])
        if has_openacc
            summary, max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med = plot_series(glob(string(root, "/iterate/model2/diffusion_benchmark_*.csv")), summary, gpu[gpu_id].caption, "acc (kernels)", "data", max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med, compare_max, compare_max.ax[3, Ngpu-gpu_id+1], compare_med, compare_med.ax[3, Ngpu-gpu_id+1])
            summary, max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med = plot_series(glob(string(root, "/iterate/model3/diffusion_benchmark_*.csv")), summary, gpu[gpu_id].caption, "acc (parallel)", "data", max_of_perf_max, min_of_perf_max, max_of_perf_med, min_of_perf_med, compare_max, compare_max.ax[4, Ngpu-gpu_id+1], compare_med, compare_med.ax[4, Ngpu-gpu_id+1])
        end

        for at in compare_max.ax[:, Ngpu-gpu_id+1]
            at.set_ylim(util_pyplot.scale_axis(min_of_perf_max, max_of_perf_max, logPlt=false))
        end
        for at in compare_med.ax[:, Ngpu-gpu_id+1]
    ...
    compare_max.fig.savefig(string("fig/", "diffusion", "_iterate", "_opt", "_max", ".png"), format="png", dpi=100, bbox_inches="tight")
    compare_med.fig.savefig(string("fig/", "diffusion", "_iterate", "_opt", "_med", ".png"), format="png", dpi=100, bbox_inches="tight")
    if output_pdf
        compare_max.fig.savefig(string("fig/", "diffusion", "_iterate", "_opt", "_max", ".pdf"), format="pdf", bbox_inches="tight")
        compare_med.fig.savefig(string("fig/", "diffusion", "_iterate", "_opt", "_med", ".pdf"), format="pdf", bbox_inches="tight")
    end
