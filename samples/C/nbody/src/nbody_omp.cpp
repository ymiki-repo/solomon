...
/// @brief benchmark of direct N-body simulation (w/o orbit integration) (implementation in OpenMP style)
///
...
write_snapshot(const int num, position *pos, velocity *vel, velocity *vel_tmp, acceleration *acc, char *file, const int snp_id, const float time, const float dt) {
  ...
  PRAGMA_OMP_TARGET_UPDATE_FROM(pos [0:num], vel [0:num], acc [0:num])
...
int main(void) {
  ...
  PRAGMA_OMP_TARGET_ENTER_DATA_MAP_ALLOC(pos [0:num], vel [0:num], acc [0:num])
  ...
  // memory deallocation
  PRAGMA_OMP_TARGET_EXIT_DATA_MAP_DELETE(pos [0:num], vel [0:num], acc [0:num])
  ...
  // measure performance
  static char filename[64];
  sprintf(filename, "log/nbody_omp_benchmark_%s%d.csv", COMPILER, MODEL_ID);
  FILE *fp;
